...
def introduce_missing(data, p_miss, uniform_miss, miss_corr):
    ...
    if uniform_miss==1:
        ...
        A = np.random.randint(0, nfeatures, size=[nsample, ])
        for i in range(nsample):
            ...
            if toomany<=0:
                ...
                mask[i,:toomany] = 1
        X_missing = np.where(mask == 1, np.nan, data)
    elif uniform_miss == 2: # used for perfect correlation results. Note, can only use 1/pmiss~= missingness
        num_fold=round(1/p_miss)
        kf = KFold(n_splits=num_fold,shuffle=True).split(range(nfeatures))
        A = np.random.randint(0, num_fold, size=[nsample, ])
        k=0
        ...
        num_lines = 2
        num_fold = 8
        boundaries = (28-num_lines*num_fold)/2 #it skips the top and bottom boundary lines, since these are often not interesting 
        assert boundaries>=0
        A = np.random.randint(0, num_fold, size=[nsample, ])*28*num_lines+28*boundaries
        for k in range(nsample):
            mask[k,A[k]:A[k]+28*num_lines]= 1
        
        X_missing = np.where(mask == 1, np.nan, data)
    else:
        raise ValueError('Incorrect missingness option. Choose variable uniform_miss from {0,...,4}')
